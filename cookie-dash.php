...
{
  // plugin version
  private static $tnhs_cookie_dash_plugin_version = '1.2.1';
  ...
  function tnhs_cookie_dash_hook_header_loggedInOnly()
  {
    ...
    if (!isset($_COOKIE["UserOptOut"]) && !isset($_COOKIE["AcceptedCookies"])) {
      add_action('wp_body_open', array(&$this, 'tnhs_cookie_dash_render_cookie_popup'));
      wp_enqueue_script('cookiespopupmanager', plugin_dir_url(__FILE__) . 'cookiespopup.min.js', array('jquery'));
    }
  ...
  function tnhs_cookie_dash_render_cookie_popup(){
...
<?php
  ...
  {
    echo('<script>console.log("If you are seeing this message, the Cookie Dash plugin was not configured correctly or the user has opted out and the Google Tag Manager container is not being loaded.");</script>');
  }
}
